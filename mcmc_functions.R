...
logLik <- function(sigma2, tau2, theta, beta) {
  ...
  dmvnorm(as.vector(newY), 
          ...
          phi %*% (C %*% solve(Cstar) %*% t(C) + tau2 * diag(nTrain)) %*% t(phi),
          log = TRUE)
}

### LOG POSTERIOR ###
logPost <- function(trSigma2, trTau2, trTheta, beta) {
  logLik(exp(trSigma2), exp(trTau2), fInv(trTheta), beta) + # Log Likelihood
    logPriorBeta(beta) + logPriorTheta(fInv(trTheta)) + # Log Priors
